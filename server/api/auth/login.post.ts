...
import useHash from "~~/server/utility/useHash";
...
export default defineEventHandler(async (event) => {
      ...
      .where(
        and(
          ...
          gt(
            ...
            new Date(Date.now() - ATTEMPT_WINDOW_SECONDS * 1000),
          ),
        ),
      );
  ...
  if (
    user.length == 0 ||
    (user.length > 0 && !compare(password, user[0].password))
  ) {
