<template>
  <section class="w-full max-h-[5500px] h-auto lg:min-h-[760px] lg:h-auto relative pt-4 px-4">
    ...
    <div class="max-w-[1440px] w-full h-[calc(100%-80px)] mx-auto sticky z-30 py-4 lg:py-6">
      <div class="w-full flex flex-col items-center justify-start gap-y-4 py-10 lg:py-20">
        ...
        <!-- FIXED SWITCHER -->
        <div class="w-auto p-2 rounded-full plan_switcher lg:my-4 flex items-center gap-2">

          <!-- Monthly -->
          <button @click="planType = true" :class="planType ? 'btn-primary' : 'btn-soft'"
            class="btn rounded-full w-28 lg:w-40 h-12 font-semibold">
            Monthly
          </button>

          <!-- Yearly -->
          <button @click="planType = false" :class="!planType ? 'btn-primary' : 'btn-soft'"
            class="btn rounded-full w-28 lg:w-40 h-12 font-semibold">
            Yearly
          </button>

          <!-- Save 5% (active when Yearly is active) -->
          <button @click="planType = false" :class="!planType ? 'btn-primary' : 'btn-soft'"
            class="btn rounded-full w-24 lg:w-28 h-12 font-semibold">
            Save 5%
          </button>
          ...
          <template v-for="(v, index) in plansTile" :key="index">
            <div :class="v.featured
              ...
              <span class="w-full flex items-center gap-x-2 relative">
                <span :class="v.bg" class="p-2 flex items-center justify-center rounded-md">
                  ...
                  <template v-for="(i, k) in plansDetails.slice(0, 8)" :key="k">
                    <tr>
                      <td class="px-0 flex items-center gap-x-2">
                        <PlansChecker size="checkbox-sm" :optionDetail="i[v.index]" />
      ...
      <div class="w-full bg-white/10 border border-primary/20 backdrop-blur-sm rounded-xl overflow-x-auto">
        <table class="table">
          <tbody>
            <template v-for="(i, k) in plansDetails.slice(0, 8)" :key="k">
              <tr class="table-tr h-16">
                <td>{{ i.Feature }}</td>
                <td><PlansChecker :optionDetail="i['Get Mailing Address Plan']" /></td>
                <td><PlansChecker :optionDetail="i['Get Business Plan']" /></td>
                <td><PlansChecker :optionDetail="i['Get Virtual Office Plan']" /></td>
              </tr>
            </template>
          </tbody>
        </table>
      ...
      <div class="w-full bg-white/10 border border-primary/20 backdrop-blur-sm rounded-xl overflow-x-auto mt-6">
        <table class="table">
          <tbody>
