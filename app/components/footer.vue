<template>
  <section class="w-full max-h-[1150px] h-auto bg-white relative">
    <div class="top_gem z-0"></div>
    <footer
      class="size-full flex flex-col max-w-[1440px] w-full mx-auto justify-center items-center sticky z-10 px-4 py-6 lg:px-10 lg:py-8"
    >
      <div
        class="w-full flex flex-col md:flex-row items-center justify-between border-b border-primary/20 pb-6 lg:pb-10"
      >
        <div class="w-full md:w-[30%] space-y-6 md:space-y-10">
          <h1 class="text-4xl font-semibold">
            Subscribe to <br class="max-lg:block hidden" />
            our newsletter
          </h1>
          <span class="flex items-center justify-between border-b">
            <input
              type="text"
              class="outline-0 w-full mr-4 h-20 text-xl text-black font-medium placeholder:pl-2"
              placeholder="Your email address"
              data-ddg-inputtype="identities.emailAddress"
              data-ddg-autofill="true"
              style="
                background-size: auto 24px !important;
                background-position: right center !important;
                background-repeat: no-repeat !important;
                background-origin: content-box !important;
                background-image: url(&quot;data:image/svg+xml;base64,&quot;) !important;
                transition: background !important;
              "
            />
            <div
              class="px-8 py-4 rounded-4xl flex items-center justify-center text-black hover:bg-primary/20"
            >
              <span class="icon-[solar--arrow-right-linear] text-3xl"></span>
            </div>
          </span>
        </div>
        <div
          class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8"
        >
          <a class="hover:text-primary" href="">Home</a>
          <a class="hover:text-primary" href="">Feature</a>
          <a class="hover:text-primary" href="">Pricing</a>
          <a class="hover:text-primary" href="">FAQ'S</a>
        </div>
        <div
          class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8"
        >
          <a class="hover:text-primary" href="">Contact Us</a>
          <a class="hover:text-primary" href="">About Us</a>
          <a class="hover:text-primary" href="">Terms &amp; Conditions</a>
          <a class="hover:text-primary" href="">Privacy Policy</a>
        </div>
        <div
          class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8 md:mt-4"
        >
          <span>123, Near opp, </span>
          <span>Some bus stand near,</span>
          <span>Tamil Nadu, India</span>
          <div class="flex items-center justify-center gap-x-4">
            <a
              target="blank"
              href="https://elg.foundation"
              class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"
            >
              <span class="icon-[mdi--globe] text-3xl"></span>
            </a>
            <a
              target="blank"
              href=""
              class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"
            >
              <span class="icon-[mdi--linkedin] text-3xl"></span>
            </a>
            <a
              target="blank"
              href=""
              class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"
            >
              <span class="icon-[mdi--instagram] text-3xl"></span>
            </a>
          </div>
        </div>
      </div>
      <div
        class="w-full flex flex-col md:flex-row items-center justify-between max-md:mt-4 lg:py-10"
      >
        <div class="space-x-4 font-medium text-lg">
          <a
            class="hover:text-primary"
            href="http://whatsai.elg.foundation/auth/login"
            >Login</a
          >
          <a
            class="hover:text-primary"
            href="http://whatsai.elg.foundation/auth/register"
            >Register</a
          >
        </div>
        <div
          class="flex items-center justify-center gap-x-2 text-lg text-primary max-md:mt-4"
        >
          <span class="icon-[solar--copyright-linear]"></span>
          <h1 class=""><b>2025 Novadesk</b> – All Rights Reserved.</h1>
        </div>
      </div>
    </footer>
  </section>
</template>
