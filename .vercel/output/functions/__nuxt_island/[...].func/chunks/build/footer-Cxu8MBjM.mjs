import { _ as __nuxt_component_0 } from './NuxtImg-BEgnGHVj.mjs';
import { _ as __nuxt_component_0$1 } from './nuxt-link-iaoeg8sI.mjs';
import { ref, withCtx, createTextVNode, unref, mergeProps, useSSRContext } from 'vue';
import { ssrRenderComponent, ssrRenderClass, ssrRenderAttrs, ssrRenderStyle } from 'vue/server-renderer';
import { a as useUserSession } from './server.mjs';
import { _ as _export_sfc } from './_plugin-vue_export-helper-ZKqNt5RG.mjs';

const _sfc_main$1 = {
  __name: "Nav",
  __ssrInlineRender: true,
  setup(__props) {
    const mobileNav = ref(false);
    const { loggedIn } = useUserSession();
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NuxtImg = __nuxt_component_0;
      const _component_NuxtLink = __nuxt_component_0$1;
      _push(`<!--[--><section class="h-auto sticky z-10 flex items-center justify-center"><nav class="navst">`);
      _push(ssrRenderComponent(_component_NuxtImg, {
        class: "max-md:h-6 md:h-10 w-auto",
        src: "/img/novadesk.webp"
      }, null, _parent));
      _push(`<ul class="nav-links"><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Home`);
          } else {
            return [
              createTextVNode("Home")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/services" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Service`);
          } else {
            return [
              createTextVNode("Service")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/plans" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Plans`);
          } else {
            return [
              createTextVNode("Plans")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/about-us" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`About Us`);
          } else {
            return [
              createTextVNode("About Us")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/contact-us" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Contact Us`);
          } else {
            return [
              createTextVNode("Contact Us")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li></ul><span class="flex items-center gap-x-3">`);
      if (!unref(loggedIn)) {
        _push(`<!--[--><button class="btn btn-ghost group/call hidden lg:flex"><span class="icon-[solar--phone-calling-linear] text-lg group-hover/call:rotate-12 transition-all duration-200 ease-linear"></span> Call Us </button><button class="btn btn-primary hidden md:flex">Lets Talk</button><button class="btn btn-primary btn-square max-md:flex hidden"><span class="icon-[solar--hamburger-menu-linear] text-lg"></span></button><!--]-->`);
      } else {
        _push(`<!--[-->`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/dashboard",
          class: "btn btn-primary max-md:hidden flex"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(` Dashboard `);
            } else {
              return [
                createTextVNode(" Dashboard ")
              ];
            }
          }),
          _: 1
        }, _parent));
        _push(`<button class="btn btn-primary btn-square max-md:flex hidden"><span class="icon-[solar--hamburger-menu-linear] text-lg"></span></button><!--]-->`);
      }
      _push(`</span></nav></section><div class="${ssrRenderClass([unref(mobileNav) ? "left-0" : "-left-full", "absolute top-0 h-dvh w-screen z-50 bg-white/10 backdrop-blur-2xl flex flex-col items-start py-6 transition-all duration-300 ease-in-out"])}"><ul class="mobile_nav w-full"><li class="w-full flex items-center justify-between"><p>Close</p><span class="icon-[material-symbols--close-rounded]"></span></li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Home`);
          } else {
            return [
              createTextVNode("Home")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/services" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Service`);
          } else {
            return [
              createTextVNode("Service")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/plans" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Plans`);
          } else {
            return [
              createTextVNode("Plans")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/about-us" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`About Us`);
          } else {
            return [
              createTextVNode("About Us")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li><li>`);
      _push(ssrRenderComponent(_component_NuxtLink, { to: "/contact-us" }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Contact Us`);
          } else {
            return [
              createTextVNode("Contact Us")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`</li></ul></div><!--]-->`);
    };
  }
};
const _sfc_setup$1 = _sfc_main$1.setup;
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/nav.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const _sfc_main = {};
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "w-full max-h-[1150px] h-auto bg-white relative" }, _attrs))}><div class="top_gem z-0"></div><footer class="size-full flex flex-col max-w-[1440px] w-full mx-auto justify-center items-center sticky z-10 px-4 py-6 lg:px-10 lg:py-8"><div class="w-full flex flex-col md:flex-row items-center justify-between border-b border-primary/20 pb-6 lg:pb-10"><div class="w-full md:w-[30%] space-y-6 md:space-y-10"><h1 class="text-4xl font-semibold"> Subscribe to <br class="max-lg:block hidden"> our newsletter </h1><span class="flex items-center justify-between border-b"><input type="text" class="outline-0 w-full mr-4 h-20 text-xl text-black font-medium placeholder:pl-2" placeholder="Your email address" data-ddg-inputtype="identities.emailAddress" data-ddg-autofill="true" style="${ssrRenderStyle({ "background-size": "auto 24px !important", "background-position": "right center !important", "background-repeat": "no-repeat !important", "background-origin": "content-box !important", "background-image": 'url("data:image/svg+xml;base64,") !important', "transition": "background !important" })}"><div class="px-8 py-4 rounded-4xl flex items-center justify-center text-black hover:bg-primary/20"><span class="icon-[solar--arrow-right-linear] text-3xl"></span></div></span></div><div class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8"><a class="hover:text-primary" href="">Home</a><a class="hover:text-primary" href="">Feature</a><a class="hover:text-primary" href="">Pricing</a><a class="hover:text-primary" href="">FAQ&#39;S</a></div><div class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8"><a class="hover:text-primary" href="">Contact Us</a><a class="hover:text-primary" href="">About Us</a><a class="hover:text-primary" href="">Terms &amp; Conditions</a><a class="hover:text-primary" href="">Privacy Policy</a></div><div class="w-full md:w-auto flex flex-col items-start justify-center gap-y-6 text-xl font-medium max-md:mt-8 md:mt-4"><span>123, Near opp, </span><span>Some bus stand near,</span><span>Tamil Nadu, India</span><div class="flex items-center justify-center gap-x-4"><a target="blank" href="https://elg.foundation" class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"><span class="icon-[mdi--globe] text-3xl"></span></a><a target="blank" href="" class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"><span class="icon-[mdi--linkedin] text-3xl"></span></a><a target="blank" href="" class="border border-gray-200 px-3 rounded-full py-3 flex items-center justify-center hover:bg-primary hover:text-white"><span class="icon-[mdi--instagram] text-3xl"></span></a></div></div></div><div class="w-full flex flex-col md:flex-row items-center justify-between max-md:mt-4 lg:py-10"><div class="space-x-4 font-medium text-lg"><a class="hover:text-primary" href="http://whatsai.elg.foundation/auth/login">Login</a><a class="hover:text-primary" href="http://whatsai.elg.foundation/auth/register">Register</a></div><div class="flex items-center justify-center gap-x-2 text-lg text-primary max-md:mt-4"><span class="icon-[solar--copyright-linear]"></span><h1 class=""><b>2025 Novadesk</b> – All Rights Reserved.</h1></div></div></footer></section>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/footer.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const __nuxt_component_3 = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]), { __name: "Footer" });

export { __nuxt_component_3 as _, _sfc_main$1 as a };
//# sourceMappingURL=footer-Cxu8MBjM.mjs.map
